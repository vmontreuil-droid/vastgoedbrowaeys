...
import { LogOut, LayoutDashboard, Users, FolderOpen, Home, Calendar, MessageSquare, Settings } from 'lucide-react'
...
const NAV = [
  ...
  { href: '/admin/aanbod',     label: 'Aanbod',     icon: Home },
...
export function AdminShell({
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--color-paper-2)' }}>
      ...
      >
        <div className="container-px mx-auto max-w-screen-2xl py-4 flex flex-wrap items-center justify-between gap-3 text-xs text-[var(--color-mute)]">
          <span>Vastgoed Browaeys — Admin</span>
          <Link href="/" className="link-underline">Bekijk publieke site →</Link>
        </div>
      </footer>
    </div>
  )
